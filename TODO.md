...
- [x] 已增强 Android 手动删除库记录：从 Library 删除 ROM 索引时同步移除同 URI 的 Recent 入口和封面缓存。
- [x] 已增强 Android 扫描来源移除：移除单个/全部 Library Folder 时同步清理对应 ROM 的 Recent 入口。
...
- [x] 已增强 Android 游戏数据包临时缓存管理：Storage 摘要和 `Clear Cache` 覆盖 `game-data-export/import`。
- [x] 已增强 Android 游戏数据包临时文件韧性：导出/导入异常中断时也会回收 state/import cache 临时文件。
- [x] 已增强 Android 用户导入文件安全性：BIOS、patch、cheat 写入统一走临时文件原子替换，失败保留旧文件并清理临时文件。
...
- [x] 已增强 Android per-game BIOS 迁移安全性：旧 game id 迁移到 stable id 时先原子复制 BIOS，再写入迁移 metadata。
- [x] 已增强 Android 日志恢复标记写入安全性：crash marker、process-exit marker 和日志裁剪改为原子替换。
...
- [x] 已完成 OnePlus7 真机 10 分钟音频长 smoke：`~/game/[gba]木叶战记.zip` 连跑 37263 native frames，Balanced 2048 buffer 下 `underruns=0`。
...
- [x] 帧率接近目标帧率，正常设备无明显 frame pacing 抖动。

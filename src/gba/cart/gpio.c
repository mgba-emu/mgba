...
static uint8_t _rtcSanitizeAndSetClockRegister(struct GBACartridgeHardware* hw, RTCDateTime index, uint8_t value);
...
void _rtcProcessByte(struct GBACartridgeHardware* hw) {
	...
	if (!hw->rtc.commandActive) {
		...
		case RTC_DATETIME:
		case RTC_TIME:
			_rtcSanitizeAndSetClockRegister(hw, 6 - hw->rtc.bytesRemaining, hw->rtc.bits);
			break;
		case RTC_ALARM:
...
void _rtcUpdateClock(struct GBACartridgeHardware* hw) {
	...
	unsigned days = diff;
	hw->rtc.time[RTC_WEEKDAY] = (hw->rtc.time[RTC_WEEKDAY] + days) % 6;
...
void _rtcSanitizeAndSetClockRegister(struct GBACartridgeHardware* hw, RTCDateTime index, uint8_t value) {

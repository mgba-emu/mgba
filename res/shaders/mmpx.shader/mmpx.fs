...
ABGR8 src(int x, int y) {
    vec2 coord = vec2(texCoord.x + float(x) / texSize.x, texCoord.y + float(y) / texSize.y);
    vec4 color = texture2D(tex, coord) * 255.;
    return 0xFF000000u | (uint(color.b) << 16) | (uint(color.g) << 8) | uint(color.r);
}

void main() {
    int srcX = int(texCoord.x);
    int srcY = int(texCoord.y);
    ...
    gl_FragColor = vec4((uvec3(N, N, N) & uvec3(0xFFu, 0xFF00u, 0xFF0000u)) >> uvec3(0, 8, 16), 255.) / 255.;
}

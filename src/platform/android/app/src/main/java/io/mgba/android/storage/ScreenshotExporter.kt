...
object ScreenshotExporter {
    fun exportToPictures(context: Context, displayName: String, writer: (ParcelFileDescriptor) -> Boolean): Uri? {
        ...
        return runCatching {
            val ok = resolver.openFileDescriptor(uri, "w")?.use(writer) == true
            if (!ok) {

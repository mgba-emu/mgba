...
// The shared portion of the mobile adapter code across the GB and GBA cores
...
#define USER1 (*(struct MobileAdapterGB*) (user))
...
static inline void serial_disable(void* user) {
	USER1.serial = 0;
}

static inline void serial_enable(void* user, bool mode_32bit) {
	USER1.serial = mode_32bit ? 4 : 1;
}

static inline bool config_read(void* user, void* dest, uintptr_t offset, size_t size) {
	return memcpy(dest, USER1.config + offset, size) == dest;
}

static inline bool config_write(void* user, const void* src, uintptr_t offset, size_t size) {
	return memcpy(USER1.config + offset, src, size) == USER1.config + offset;
}

static inline bool sock_open(void* user, unsigned conn, enum mobile_socktype type, enum mobile_addrtype addrtype, unsigned bindport) {
	Socket fd;
...
static inline void sock_close(void* user, unsigned conn) {
...
static inline int sock_connect(void* user, unsigned conn, const struct mobile_addr* addr) {
...
static inline bool sock_listen(void* user, unsigned conn) {
	if (SOCKET_FAILED(USER1.socket[conn].fd)) {
		Socket fd;
		struct Address bindaddr, *bindptr = NULL;
...
static inline bool sock_accept(void* user, unsigned conn) {
...
static inline int sock_send(void* user, unsigned conn, const void* data, unsigned size, const struct mobile_addr* addr) {
	if (addr) {
...
static inline int sock_recv(void* user, unsigned conn, void* data, unsigned size, struct mobile_addr* addr) {
	int res;
...
static inline void update_number(void* user, enum mobile_number type, const char* number) {
